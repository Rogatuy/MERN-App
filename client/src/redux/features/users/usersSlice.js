...
export const getAllUsers = createAsyncThunk(
  ...
  try {
    const {data} = await axios.get('https://homework-4-itransition.herokuapp.com/users');
    return data;
  } catch (error) {
    console.log(error);
  }
...
export const removeUser = createAsyncThunk(
  ...
  async (_id) => { 
    try{
      const {data} = await axios.delete('https://homework-4-itransition.herokuapp.com/users/delete', {
        data:{_id}
      });
...
export const blockedUser = createAsyncThunk(
  ...
  async ({_id}) => { 
    try{
      const {data} = await axios.post('https://homework-4-itransition.herokuapp.com/users/blocked', {
        _id,
      });
...
export const unblockedUser = createAsyncThunk(
  ...
  async ({_id}) => { 
    try{
      const {data} = await axios.post('https://homework-4-itransition.herokuapp.com/users/unblocked', {
        _id,
      });
